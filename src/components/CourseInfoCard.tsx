...
const CourseInfoCard: React.FC<CourseInfoCardProps> = ({
  ...
  // Whether we actually have any non-zero enrollment data yet
  const hasEnrollmentData =
    !!enrollmentData &&
    !!(
      enrollmentData.currentEnrollment ||
      enrollmentData.pastEnrollment ||
      enrollmentData.yearAgoEnrollment ||
      enrollmentData.threeTermsAgoEnrollment
    );
  ...
  const getThreadColor = (threadName: string): string => {
    ...
    const FALLBACK_COLORS: Record<string, string> = {
      ...
      science: '#22c55e',
      'seas-core': '#fbbf24',
      'cs-intro-core': '#0ea5e9',
      'prob-stat': '#fb923c',
      'cs-area-foundation': '#a855f7',
      'cs-elective': '#ec4899',
      'tech-elective': '#64748b',

      // OLD THREAD NAMES (in case any course still uses them)
      required: '#bb7f00',
      intel: '#017dbe',
      info: '#91a70d',
      people: '#268f3b',
        ...
        <CardContent className="pt-0 pb-3 px-3">
          ...
          {isLoading ? (
            ...
            <div className="mt-2 text-[10px] text-gray-400 dark:text-gray-500 italic">
              Live enrollment statistics coming soon — UI is ready, awaiting data source.
            </div>
          )}
        </CardContent>
      </Card>
